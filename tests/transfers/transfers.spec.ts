...
test('Should transfer money successfully between own accounts', { tag: ['@smoke', '@regression'] }, async ({
  ...
  await transfersPage.transfer({
    sourceAccount: accounts.checkingAccount.accountCode,
    destinationAccount: accounts.savingsAccount.accountCode,
    amount: '100',
  });
  ...
  await expect(
    toastNotification.getMessage(messages.successTransfer),
  ).toBeVisible();
});
...
test('Should display an error message when trying to transfer between the same accounts', { tag: '@regression' }, async ({
  ...
  await transfersPage.transfer({
    sourceAccount: accounts.checkingAccount.accountCode,
    destinationAccount: accounts.checkingAccount.accountCode,
    amount: '100',
  });
  await expect(transfersPage.transferErrorMessage).toHaveText(
    new RegExp(messages.sameAccounts),
  );
});
...
test('Should display an error message when trying to transfer more than the limit', { tag: '@regression' }, async ({
  ...
  await transfersPage.transfer({
    sourceAccount: accounts.checkingAccount.accountCode,
    destinationAccount: accounts.savingsAccount.accountCode,
    amount: String(amountOutsideLimit),
  });
  const transferConfirmationModal = new TransferConfirmationModal(page);
  await transferConfirmationModal.confirmButton.click();
  await expect(transfersPage.transferErrorMessage).toHaveText(
    new RegExp(escapeRegExp(messages.amountMax)),
  );
});
...
test('Should display an error message when trying to transfer an empty amount', { tag: '@regression' }, async ({
  ...
  const result = await transfersPage.transferAmount.evaluate(
