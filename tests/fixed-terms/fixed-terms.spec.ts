...
test('Should deposit created successfully', async ({ dashboardPage, page }) => {
  ...
  const amount = '15000';
  await fixedTermsPage.createDeposit({
    sourceAccount: accounts.checkingAccount.accountCode,
    amount,
    term: fixedTerms.thirtyDays.days,
  });
  ...
  await expect(
    toastNotification.getMessage(messages.successDeposit),
  ).toBeVisible();
});
...
test('Should deposit cancelled successfully', async ({
  ...
  await expect(
    toastNotification.getMessage(messages.cancelledDeposit),
  ).toBeVisible();
});
